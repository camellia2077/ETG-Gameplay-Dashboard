...
The selected room refresh mode is persisted as `RoomEnemyRefreshMethod` (`rewind` or `respawn`). The selected mode applies to standard combat rooms only; Boss rooms always use rewind. The rewind shortcut is edited in the Room page and persisted as `room.rewind=KeyCode` in `KeyboardShortcuts`, with `C` as the default. The in-panel execute button does not persist a pressed state.
