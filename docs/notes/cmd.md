## Debug 

python .\tools\deploy\deploy_mod.py "C:\Game\steam\steamapps\common\Enter the Gungeon" --configuration Debug

覆盖config(包括文本资源)
python .\tools\deploy\deploy_mod.py "C:\Game\steam\steamapps\common\Enter the Gungeon" --configuration Debug --overwrite-config

## Release

python .\tools\deploy\deploy_mod.py "C:\Game\steam\steamapps\common\Enter the Gungeon" --configuration Release --overwrite-config




如果你想立刻用最新功能，最简单的是继续部署 Debug，因为它已经是最新的：

python .\tools\deploy\deploy_mod.py "C:\Game\steam\steamapps\common\Enter the Gungeon"

python .\tools\deploy\deploy_mod.py "C:\Game\steam\steamapps\common\Enter the Gungeon" --overwrite-config





