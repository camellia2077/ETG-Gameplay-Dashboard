...
`Reveal Map` is the runtime ON/OFF control. Turning it ON in the foyer still reveals the foyer immediately, and also arms an automatic reveal for the first true dungeon floor. In `Current Floor` mode that pending foyer request is consumed once; in `Every Floor` mode it becomes the automatic reveal for every subsequent dungeon floor. Using `General -> Teleport` to leave the foyer preserves the pending foyer request, while teleporting between dungeon floors still resets the current-floor scope. Turning it ON directly in a dungeon reveals the current floor, with the same scope rules for later floors. Turning it OFF clears the pending request and stops future automatic reveals. The selected scope is persisted in `RevealMapEveryFloor`; the runtime ON/OFF and pending foyer request are intentionally controlled by the General-page button.
