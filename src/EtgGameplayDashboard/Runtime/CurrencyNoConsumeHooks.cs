    ...
    internal static class CurrencyNoConsumeHooks
    {
        ...
        public static void Install(Harmony harmony, ManualLogSource logger)
        {
            ...
            if (targetMethod == null || prefixMethod == null || postfixMethod == null)
            {
                if (logger != null)
                {
                    logger.LogWarning(EtgGameplayDashboardLog.Init("Currency no-consume hook skipped: " + hookLabel + ". Target or patch method was not found."));
                }
            ...
            try
            {
                ...
                if (logger != null)
                {
                    logger.LogInfo(EtgGameplayDashboardLog.Init("Currency no-consume hook ready: " + hookLabel));
                }
            }
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.LogWarning(EtgGameplayDashboardLog.Init("Currency no-consume hook failed: " + hookLabel + ". " + ex.GetType().Name + ": " + ex.Message));
                }
            }
        }
