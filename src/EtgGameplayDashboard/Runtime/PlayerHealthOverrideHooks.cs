    ...
    internal static class PlayerHealthOverrideHooks
    {
        ...
        public static void Install(Harmony harmony, ManualLogSource logger)
        {
                ...
                removeArmorMethod == null || removeArmorPrefix == null || removeArmorPostfix == null)
            {
                if (logger != null)
                {
                    logger.LogWarning(EtgGameplayDashboardLog.Init("Player health override hook skipped: " + hookLabel + ". Target or patch method was not found."));
                }
            ...
            try
            {
                ...
                if (logger != null)
                {
                    logger.LogInfo(EtgGameplayDashboardLog.Init("Player health override hook ready: " + hookLabel));
                }
            }
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.LogWarning(EtgGameplayDashboardLog.Init("Player health override hook failed: " + hookLabel + ". " + ex.GetType().Name + ": " + ex.Message));
                }
            }
        }
