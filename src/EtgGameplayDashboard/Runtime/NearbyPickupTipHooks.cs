    ...
    internal static class NearbyPickupTipHooks
    {
        ...
        private static void Patch(Harmony harmony, ManualLogSource logger, Type targetType, string methodName, Type[] parameterTypes, string hookLabel, string postfixMethodName)
        {
            ...
            if (targetMethod == null || postfixMethod == null)
            {
                if (logger != null)
                {
                    logger.LogWarning(EtgGameplayDashboardLog.Init("Nearby pickup tip hook skipped: " + hookLabel + ". Target or patch method was not found."));
                }
            ...
            try
            {
                ...
                if (logger != null)
                {
                    logger.LogInfo(EtgGameplayDashboardLog.Init("Nearby pickup tip hook ready: " + hookLabel));
                }
            }
            catch (Exception exception)
            {
                if (logger != null)
                {
                    logger.LogWarning(EtgGameplayDashboardLog.Init("Nearby pickup tip hook failed: " + hookLabel + ". " + exception.GetType().Name + ": " + exception.Message));
                }
            }
        }
