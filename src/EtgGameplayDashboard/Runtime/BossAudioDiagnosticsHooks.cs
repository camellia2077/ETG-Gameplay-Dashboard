    ...
    internal static class BossAudioDiagnosticsHooks
    {
        ...
        private static void Install(Harmony harmony, Type type, string targetName, Type[] args, string patchName, bool prefix, ManualLogSource logger)
        {
            ...
            if (target == null || patch == null)
            {
                if (logger != null)
                {
                    logger.LogWarning(EtgGameplayDashboardLog.Init("Boss audio diagnostics hook skipped: " + type.Name + "." + targetName + "."));
                }
            ...
            if (logger != null)
            {
                logger.LogInfo(EtgGameplayDashboardLog.Init("Boss audio diagnostics hook ready: " + type.Name + "." + targetName + "."));
            }
        }
