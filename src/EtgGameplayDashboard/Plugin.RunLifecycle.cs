    ...
    public sealed partial class Plugin
    {
        ...
        private void TryHandleCurrentScene(string source)
        {
            ...
            if (lifecycle.SceneChanged)
            {
                ...
                Logger.LogInfo(EtgGameplayDashboardLog.Run("Observed scene change via " + source + ": " + lifecycle.PreviousSceneName + " -> " + lifecycle.SceneName));
            ...
            if (lifecycle.ResetKind == RunLifecycleResetKind.PrimaryPlayerChanged)
            {
                ...
                {
                    Logger.LogInfo(EtgGameplayDashboardLog.Run("Detected a new PrimaryPlayer instance in scene " + lifecycle.SceneName + ". Resetting run grant state."));
                }
            ...
            if (lifecycle.ResetKind == RunLifecycleResetKind.EnteredCharacterSelectHub)
            {
                if (_runState.HasGrantedThisRun || _runState.CurrentSeed != 0)
                {
                    Logger.LogInfo(EtgGameplayDashboardLog.Run("Entered character select hub. Resetting run grant state."));
                }
            ...
            if (_bossRushService != null && _bossRushService.ShouldSuppressAutomaticLoadout)
            {
                if (lifecycle.SceneChanged)
                {
                    Logger.LogInfo(EtgGameplayDashboardLog.Run("Boss Rush is active. Suppressing automatic random loadout for scene " + lifecycle.SceneName + "."));
                }
            ...
            if (lifecycle.ShouldScheduleGrant)
            {
                ...
                {
                    Logger.LogInfo(EtgGameplayDashboardLog.Run("PrimaryPlayer changed inside scene " + lifecycle.SceneName + ". Delaying loadout grant by " + GrantDelaySeconds + " seconds."));
                }
                else
                {
                    Logger.LogInfo(EtgGameplayDashboardLog.Run("Scene " + lifecycle.SceneName + " entered. Delaying loadout grant by " + GrantDelaySeconds + " seconds."));
                }
            }
            ...
            if (!_enableEtgGameplayDashboardConfig.Value)
            {
                if (lifecycle.SceneChanged)
                {
                    Logger.LogInfo(EtgGameplayDashboardLog.Run("Automatic random loadout is disabled by config."));
                }
            ...
            if ((object)player == null)
            {
                if (lifecycle.SceneChanged)
                {
                    Logger.LogInfo(EtgGameplayDashboardLog.Run("Scene " + lifecycle.SceneName + " is active, but PrimaryPlayer is not ready yet."));
                }
        ...
        private bool BeginDeferredTeleportFromFoyer(EtgFloorDefinition floorDefinition, string labelKey, string commandText)
        {
            ...
            {
                Logger.LogWarning(EtgGameplayDashboardLog.Command("Deferred teleport could not start because the Keep floor definition is unavailable."));
                return false;
            }
        ...
        private void LogFloorTeleportRunInfo(string message)
        {
            ...
            Logger.LogInfo(EtgGameplayDashboardLog.Run(message));
        }
