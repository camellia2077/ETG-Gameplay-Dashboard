    ...
    internal static class ControllerAimLockHooks
    {
        ...
        public static void Install(Harmony harmony, ManualLogSource logger)
        {
                ...
                cameraOffsetTarget == null || cameraOffsetPrefix == null || cameraOffsetPostfix == null)
            {
                if (logger != null)
                {
                    logger.LogWarning(RandomLoadoutLog.Init("Controller aim lock hook skipped: PlayerController.DetermineAimPointInWorld was not found."));
                }
            ...
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.LogWarning(RandomLoadoutLog.Init("Controller aim lock hook failed: " + ex.GetType().Name + ": " + ex.Message));
                }
            }
        }
        ...
        private static void LogAimSample(PlayerController player, Vector3 rawAimPoint)
        {
            ...
            s_logger.LogInfo(RandomLoadoutLog.Aim(
            ...
            bool lockActive)
        {
            ...
            s_logger.LogInfo(RandomLoadoutLog.CameraAim(
                "Sample Frame=" + Time.frameCount +
