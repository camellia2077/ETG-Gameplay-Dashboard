    ...
    internal static class BossIntroSkipHooks
    {
        ...
        public static void Install(Harmony harmony, ManualLogSource logger, System.Func<bool> verboseLoggingEnabledProvider)
        {
            ...
            if (target == null || patch == null || enteredTarget == null || enteredPatch == null || triggerTarget == null || triggerPatch == null || IsRunningField == null || SkipDelayField == null)
            {
                if (logger != null)
                {
                    logger.LogWarning(RandomLoadoutLog.Init("Boss intro skip hook skipped because GenericIntroDoer members were unavailable."));
                }
            ...
            {
                logger.LogWarning(RandomLoadoutLog.Init("Boss intro skip could not hook BossTriggerZone."));
            }

            if (logger != null)
            {
                logger.LogInfo(RandomLoadoutLog.Init("Boss intro skip hook ready: GenericIntroDoer.InvariantUpdate postfix."));
            }
        }
        ...
        private static void BossTriggerZonePrefix(BossTriggerZone __instance)
        {
            ...
            {
                s_logger.LogWarning(RandomLoadoutLog.Command("Boss intro skip entered a BossTriggerZone but found no matching GenericIntroDoer."));
            }
        }
        ...
        private static IEnumerator WaitForBossIntroReady_CR(GenericIntroDoer introDoer)
        {
            ...
            if (IsVerboseLoggingEnabled && s_logger != null)
            {
                s_logger.LogWarning(RandomLoadoutLog.Command(BuildIntroTimeoutMessage(introDoer)));
            }
