    ...
    internal static class AmmonomiconAnimationHooks
    {
        ...
        public static void Install(Harmony harmony, ManualLogSource logger)
        {
            ...
            if (targetMethod == null || patchMethod == null)
            {
                if (logger != null)
                {
                    logger.LogWarning(RandomLoadoutLog.Init("Ammonomicon animation hook skipped: " + hookLabel + ". Target or patch method was not found."));
                }
            ...
            try
            {
                ...
                if (logger != null)
                {
                    logger.LogInfo(RandomLoadoutLog.Init("Ammonomicon animation hook ready: " + hookLabel));
                }
            }
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.LogWarning(RandomLoadoutLog.Init("Ammonomicon animation hook failed: " + hookLabel + ". " + ex.GetType().Name + ": " + ex.Message));
                }
            }
        }
